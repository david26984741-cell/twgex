...
| 週一～五 15:15 | TXO ＋ 美股四檔 | 台指一般時段 13:45 收盤、期交所盤後檔約 15:00 上架；順便當美股那班的補跑 |
| 週二～六 15:15 ／ 21:00 | ES | 跑在自架 runner（紅線那台）上，見下。**時間受那台的外網時段限制**，兩班互為補跑 |
...
**【最重要的一條：那台的外網只在台北 14:00 ~ 隔天 06:00 開放】**
06:00~14:00 公司封外網，這段時間 runner 連得到 GitHub、抓得動 repo，但**打不到 CME**。
所以 ES 的排程一定要整個避開那八小時。

實測證據（2026/09/04~08）：三次在封網時段起跑的執行（13:13、13:26、13:34）
**全部在 14:04~14:05 結束**——不是「跑了 30~50 分鐘」，是卡在那邊對著封閉的網路
重試，等到 14:00 開網才在五分鐘內跑完。完全落在開放時段的執行都只要 **4~5 分鐘**。
舊的台北 09:00 那班永遠落在封網中，結構上不可能成功，已經移除。

現在排 **台北 15:15 與 21:00**，兩班都留大量餘裕：GitHub 排程常延遲一到三小時，
15:15 延遲四小時是 19:15、21:00 延遲四小時是隔天 01:00，都還在開放時段內。
（試過在每一輪前先用 `curl` 探一次 CME 通不通，**撤掉了**：探測一律回 HTTP 000，
但同一時間 `build.py` 抓得好好的——Akamai 看 User-Agent，curl 預設的 `curl/8.x`
會被直接重置連線。假警報還把整條線擋死。要重做的話 UA 必須跟 `cme.py` 一致，
而且要先實測過再放進來。）
