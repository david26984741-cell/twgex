...
| 週一～五 23:00 | SPX / SPY / QQQ | 美東夏令 11:00 / 冬令 10:00，抓的是**前一個**美股收盤 |
| 週一～五 21:20 | ES | 不在 Actions 裡，另外由排程任務驅動瀏覽器抓（見下） |

**為什麼美股那班排在開盤之後。** 這一點跟直覺相反，是踩了兩次坑才弄清楚的：
CBOE 那份檔案裡，價格是即時的，未平倉量卻要等 OCC 隔天早上才更新（實測美東 10:00~10:30）。
所以開盤前抓 → 價格對但未平倉舊一天；等未平倉更新 → 未平倉對了但價格變成盤中。
**沒有任何一個時點兩者同時正確。**

解法是價格改取每一檔的 `prev_day_close`（前一交易日收盤價），
於是只剩「未平倉已更新」這一個條件，而那個窗口一路開到當天收盤，
有五個多小時，排程延遲一兩小時都無所謂。
workflow 裡的 `wait_oi.py` 會先確認未平倉真的更新到前一個交易日才開始建圖；
等不到就整天不產出，也不會畫出「舊未平倉 ＋ 新價格」的拼裝圖。
產出的 `meta` 裡 `oi_as_of` 與 `price_as_of` 必須相同，不同就是有問題。
