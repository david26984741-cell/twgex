...
| 週一～五 20:20 | SPX / SPY / QQQ | 美東夏令 08:20 / 冬令 07:20，抓的是**前一個**美股收盤 |
| 週一～五 21:20 | ES | 不在 Actions 裡，另外由排程任務驅動瀏覽器抓（見下） |

**為什麼美股那班排在開盤前 70 分鐘而不是 10 分鐘。** GitHub 的排程常延遲 10~60 分鐘。
原本排在美東 09:20，2026/08/24 那次被延到 09:51 才跑——已經開盤 21 分鐘，
於是產出變成「前一日的未平倉量 ＋ 當日盤中的報價」，SPX 的總 GEX 從 +7,344 變成 −17,090。
資料本身有誠實記錄（`meta.oi_as_of` / `price_as_of` / `quote_time`），網頁也會跳出警示，
但那不是原本要的東西。往前挪一小時就是為了留緩衝。
冬令時這一班會落在美東 07:20，若 OCC 當天發布得晚，可能拿到再前一天的未平倉量——
程式會照實標示，不會假裝是新的。
